...
Importantly, pretraining is a general method to pass information from one model to another -- it has many uses beyond what has already been discussed here, including time series data, multi-response data and conditional average treatment effect estimation. Some of these modeling tasks are not supported by the `ptLasso` package, and the final section in this vignette shows how to do pretraining for them using the `glmnet` package. 
